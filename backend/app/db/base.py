...
class ProvenanceColumns:
    """Citation anchor carried on every extracted row (see schemas.common.Provenance)."""

    clause_ref: Mapped[str | None] = mapped_column(nullable=True)
    source_page: Mapped[int | None] = mapped_column(nullable=True)
    source_snippet: Mapped[str | None] = mapped_column(nullable=True)
