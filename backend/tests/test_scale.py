...
from app.compliance.matching import _embed_one, match_document
...
def test_repeated_reports_reuse_the_cache():
    ...
    requirements = [f"Required item {i}" for i in range(10)]
    _embed_one.cache_clear()
    ...
    with patch(
        ...
        for requirement in requirements:
            match_document(requirement, ["Vendor A enclosure"])
        first = _embed_one.cache_info().misses
        for requirement in requirements:
            match_document(requirement, ["Vendor A enclosure"])
        second = _embed_one.cache_info().misses
